...
public record NoticeFileUploadPresignedUrlCreateResponse(
    ...
    public record NoticeFileUploadPresignedUrlCreateResponseItem(
            ...
            String fileUrl                  //조회용 주소 (https://dev-cdn.edukit.co.kr/notices/20250725_223935_ea5f18be.jpg)
    ) {
        public static NoticeFileUploadPresignedUrlCreateResponseItem of(final long noticeFileId, final String presignedUrl, String fileUrl) {
            return new NoticeFileUploadPresignedUrlCreateResponseItem(noticeFileId, presignedUrl, fileUrl);
        }
    }

    public static NoticeFileUploadPresignedUrlCreateResponse of(List<UploadPresignedUrlResponse> presignedUrls, List<NoticeFile> noticeFiles) {
        ...
        List<NoticeFileUploadPresignedUrlCreateResponseItem> items = presignedUrls.stream()
                .map(urlResponse -> {
                    NoticeFile noticeFile = filePathToNoticeFile.get(urlResponse.s3Key());
                    return NoticeFileUploadPresignedUrlCreateResponseItem.of(noticeFile.getId(), urlResponse.presignedUrl(), urlResponse.fileUrl());
                })
                .toList();
